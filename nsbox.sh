...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -r|--root)
            if [ -n "$2" ]; then
                if [ ! -d "$2" ]; then
                    echo "Ошибка: Корневой каталог '$2' не найден на хосте!"
                    exit 1
                fi
                ROOT_DIR=$(realpath "$2")
                shift 2
            else
                echo "Ошибка: Параметр --root требует указания пути к каталогу."; exit 1
            fi
            ;;
        -n|--net)
...
if [ -z "$CONTAINER_NAME" ]; then
    echo -n "Ошибка: Не указано имя контейнера. "
    existed_containers=$(find "$STORAGE_DIR" -type f -name *.img -print0 | xargs -0 -n 1 basename -s .img 2>/dev/null | tr '\n' ' ')
    if [ -n "$existed_containers" ]; then
        echo -n "Укажите новый или один из существующих: $existed_containers"
    fi
    echo -e "\n"
    usage
fi
...
IMAGE_FILE="$STORAGE_DIR/$CONTAINER_NAME.img"
...
# Монтирование виртуального диска для upperdir
if ! mountpoint -q "$UPPER_DIR"; then
    mount -o loop,user_xattr "$IMAGE_FILE" "$UPPER_DIR"
fi
...
# Создание каталога для старого корня при рокировке корней
...
if [ "$IS_NET_ENABLED" = true ]; then
    ...
    # Принудительное удаление файла дескриптора netns, если он "застрял" в памяти
    ...
    # В некоторых версиях ubuntu /etc/resolv.conf это симлинк на /run/resolfconf/resollf.conf и это не работает!
...
if [ -n "$PORT_MAP" ]; then
    ...
    nft add chain ip nsbox_nat postrouting { type nat hook postrouting priority 100 \; } 2>/dev/null || true
    # Настройка маскарадинга источника для корректного возврата ответов от контейнера к хосту
...
umount -l "$UPPER_DIR"   2>/dev/null || true
