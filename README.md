...
Контейнер использует оверлей, создаваемый в каталоге `/var/lib/rigger/<container_name>` и обычной структурой подкаталогов: `lower`, `upper`, `work`, `merged`. Перед запуском в каталоге lower необходимо развернуть корневую файловую систему контейнера. Если при запуске контейнера каталог нижнего слоя отсутствует или пустой, в него автоматически монтируется корень файловой системы хоста.
