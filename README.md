...
Для использования `nsbox` без sudo, добавить пользователя в группу `nsbox` и `systemd-journal`. Пользователям этой группы разрешено управлять службой `nsbox@.service` и просматривать журнал без повышения привилегий.

Пи запуске `nsbox` проверяет подкаталог `/var/lib/machines/<container_name>`, если он пустой,  то создает в каталоге `/var/lib/nsbox/<container_name>`  подкаталоги `lower`, `upper`, `work`, в `lower` монтирует корень хоста и монтирует оверлей в `/var/lib/machines/<container_name>`.

Для создания файловой системы контейнера в `/var/lib/machines/<container_name>` можно использовать команду `load` для загрузки и развертывания образов docker или архивов. Также можно создать контейнер командой `create` и развернуть в `/var/lib/machines/<container_name>` корень с помощью утилит типа `debootstrap`.
...
Из секции `[Exec]` при отключенной загрузке системы __игнорируются__ следующие параметры:
...
```
# Установка пакет локалей
apt-get update && apt-get install -y locales

# Раскомментирование русской кодировки в системном файле
sed -i 's/# ru_RU.UTF-8 UTF-8/ru_RU.UTF-8 UTF-8/' /etc/locale.gen

# Компиляция локали
locale-gen ru_RU.UTF-8

# Установка локали глобально для системы контейнера
update-locale LANG=ru_RU.UTF-8
```

# Сводка по основным командам machinectl

## Информация и статус
machinectl list — Показать все активные (запущенные) контейнеры, их статус, нагрузку и IP-адреса.
machinectl list-images — Показать все доступные образы (шаблоны/скачанные контейнеры) в /var/lib/machines.
machinectl status <имя> — Вывести подробный статус контейнера (UID, PID, потребление памяти, дерево процессов cgroup и последние логи).
machinectl show <имя> — Показать низкоуровневые свойства и параметры контейнера в формате КЛЮЧ=ЗНАЧЕНИЕ.

## Управление жизненным циклом (Контейнеры)
machinectl start <имя> — Запустить контейнер в фоновом режиме (через системную службу systemd-nspawn@имя.service).
machinectl stop <имя> — Безопасно завершить работу контейнера (отправляет сигнал выключения внутрь ОС контейнера).
machinectl poweroff <имя> — Принудительно и немедленно выключить контейнер (аналог выдергивания шнура питания).
machinectl reboot <имя> — Перезагрузить контейнер.
machinectl terminate <имя> — Завершить все процессы контейнера и освободить его ресурсы (жесткая очистка).

## Доступ и выполнение команд
machinectl shell [<пользователь>@]<имя> [<команда>] — Открыть интерактивную сессию терминала внутри контейнера. Если команда не указана, открывает дефолтный bash/sh.
machinectl login <имя> — Открыть классический текстовый сеанс входа (запрашивает логин и пароль, как на физической TTY-консоли).
machinectl bind <имя> <путь_хоста> [<путь_контейнера>] — На лету (без перезапуска) примонтировать директорию с хост-системы внутрь запущенного контейнера.

## Обмен файлами между хостом и контейнером
machinectl copy-to <имя> <путь_на_хосте> <путь_в_контейнере> — Скопировать файл или папку с хоста внутрь запущенного контейнера.
machinectl copy-from <имя> <путь_в_контейнере> <путь_на_хосте> — Скопировать файл или папку из контейнера наружу на хост.

## Дистрибуция и импорт/экспорт образов
machinectl pull-tar <URL> [<имя>] — Скачать архив .tar по сети, автоматически проверить его и развернуть как новый контейнер.
machinectl import-tar <файл> [<имя>] — Импортировать и распаковать локальный .tar архив в /var/lib/machines.
machinectl export-tar <имя> [<файл>] — Упаковать существующий контейнер в .tar архив для бэкапа или переноса.
machinectl list-transfers — Показать статус текущих фоновых скачиваний или операций импорта/экспорта.
machinectl cancel-transfer <ID> — Отменить активный процесс скачивания или импорта.

##Управление образами (Файловая система)
machinectl clone <имя_образа> <новое_имя> — Клонировать образ/контейнер. На файловых системах вроде Btrfs это происходит мгновенно за счет рефлинков.
machinectl rename <старое_имя> <новое_имя> — Переименовать контейнер или образ.
machinectl remove <имя> — Полностью удалить образ или контейнер из /var/lib/machines и очистить место на диске.
machinectl read-only <имя> [true|false] — Установить или снять флаг "только для чтения" для файловой системы контейнера.

# Утилита importctl
Когда ваш дистрибутив обновится до systemd 260+, команда importctl pull-oci станет доступна.
Она будет работать с Docker Hub напрямую через следующий синтаксис:
```
importctl pull-oci docker://ubuntu:latest my-ubuntu --verify=no
```
