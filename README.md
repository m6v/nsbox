...
При запуске контейнера в режиме `Boot=no` (без флага `-b`) утилита `systemd-nspawn` обрабатывает секцию `[Exec]` крайне избирательно. Большая часть параметров, управляющих окружением, игнорируется, но некоторые критические системные директивы продолжают работать.
