# Коллекция простых скриптов на разные случаи жизни
...
EOF
```
> Уточнить по логам, принимается ли в Астре параметр ResolvConf. В debian нет!
...
## nsbox.sh и nsinit.sh
Реализация контейнеров на bash с помощью нативных средств ядра Linux и оберток над ними (`unshare` и `pivot_root`)

## ifinit.sh
Скрипт, позволяющий по заданной таблице MAC-адресов восстановить имена и IP-адреса хостов

## getperms и chkperms
Pyton- и bash-скрипты создания списка разрешений объектов доступа и проверки соответствия установленных разрешений.
